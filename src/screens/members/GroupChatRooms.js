...
export default GroupChatRooms = ({ route, navigation }) => {
    ...
    const onResult = (querySnapshot) => {
        ...
        const chatRoomsUni = [...new Map(querySnapshot.docs.map(item =>
            [item.data().chatRoom, item])).values()];
    ...
    useEffect(() => {
        Action.members.list({}).then(response => {
            ...
            <ScrollView style={styles.contentContainer} contentContainerStyle={{ paddingBottom: 30 }}>
                {
                    chatRooms.map((item, index) => (
                        <TouchableOpacity key={index} onPress={() => goIntoChatRoom(item.data().chatRoom, item.data().members)}>
                            <View style={styles.card}>
                                <Text style={globalStyles.boldLabel}>{item.data().chatRoom}</Text>
                                <Text style={[globalStyles.boldSmallLabel, { marginLeft: 20 }]}>{item.data().members.length} members</Text>
                            </View>
                        </TouchableOpacity>
                    ))
                }
